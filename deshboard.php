...
<body>
    ...
    <div class="heading">
        ...
        $iyvalue = [];
        $Accno = $_SESSION['Accno'];
        $i = "select cetegory,sum(amount) as cval from transactions where account = $Accno and type = 'Income' and MONTH(tran_date) = '$d' and
     year(tran_date) = year(now()) group by cetegory";
        ...
        $eyvalue = [];
        $Accno = $_SESSION['Accno'];
        $i = "select cetegory,sum(amount) as cval from transactions where account = $Accno and type = 'Expense' and MONTH(tran_date) = '$d' and
     year(tran_date) = year(now()) group by cetegory";
        $iq = mysqli_query($con, $i)
            or
            die("Error in select income data");
        ...
        echo "<div class='Balance'>
            <h3>Balance</h3>
            <abbr title='Total Income - Total Expense'>
            <p> " . $_SESSION['country_symbol'] . " " . $_SESSION['Balance'] . "/-</p>
            </abbr></div> 
        </div> ";
        ...
        $e = "select DISTINCT MONTHNAME(tran_date) as months,sum(amount) as expense from transactions where account = $Accno and type = 'Expense' and
    year(tran_date) = year(now()) group by MONTH(tran_date) order by tran_date ";
        ...
        $i = "select DISTINCT MONTHNAME(tran_date) as months,sum(amount) as income from transactions where account = $Accno and type = 'Income' and
     year(tran_date) = year(now()) group by MONTH(tran_date) order by tran_date";
        $iq = mysqli_query($con, $i)
            or
            die("Error in select income data");
...
*/
    ...
    <div class="content">
        ...
        <div class="twograph">
            ...
            <p for="date" id="graph-date" align="center"></p>
            ...
            <script>
                ...
                const startDate = new Date(currentYear, 0, 2); // January is month 0
                const endDate = new Date(currentYear, 11, 32); // December is month 11
                // Format to YYYY-MM-DD
                const formatDate = (date) => date.toISOString().split('T')[0];
                document.getElementById('graph-date').innerHTML = ("From " + formatDate(startDate) + " To " + formatDate(endDate));
        ...
        <div class="monthdetail">
            ...
            <div class="monIncm">
                <?php
                include "connect.php";
                echo "<h3>Income " . date('F') . " " . date('Y') . "</h3>";
                echo "<p>" . $_SESSION['country_symbol'] . " ";
                $d = date("m ");
                $tim = 0;
                $im = "select amount from income where account = $Accno and MONTH(trandate) = $d and year(trandate) = year(now())";
            ...
            <div class="monExpnc">
                <?php
                include "connect.php";
                echo "<h3>Expense " . date('F') . " " . date('Y') . "</h3>";
                echo "<p>" . $_SESSION['country_symbol'] . " ";
                $d = date("m ");
                $t = 0;
                $tr = "select amount from expense where account = $Accno and MONTH(trandate) = $d and year(trandate) = year(now())";
